...
manager). When in doubt about how to grow VATWorksheet into a real feature,
look there.
...
VATWorksheet demonstrates the whole loop with the least possible surface:
one endpoint (`GET /api/ping`, returning `"pong"`) and one page with a Ping
button that calls it and shows the response.

## Auth

VATWorksheet ships with **no auth** — every endpoint and page is public.
...
```
VATWorksheet.csproj          Web SDK project; wwwroot copied to output for Lambda
...
1. Copy the folder, then rename `VATWorksheet.csproj`, `VATWorksheet.slnx` (and the
   project path inside it), and the `VATWorksheet` identifiers in `Program.cs`
